...
import (
	...
	"github.com/AlindHere/crood/pkg/build"
	"go.uber.org/zap"
...
func init() {
	...
	f.StringVarP(&config.VersionExtra, "version-extra", "e", "", "extra will be added to the build version if the ")
	f.StringVarP(&config.VersionScheme, "version-scheme", "s", "long", "scheme can be 'short' or 'long', short= ${version-tag}, long= ${version-tag}-${version-extra}.${build-id}")
